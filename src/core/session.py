...
def mfa_pendente_required(f):
    ...
    ALTERADO: além de exigir id_usuario + mfa_pendente na sessão,
    agora também expira o estado por TEMPO FIXO desde a criação
    (TTL_MFA_PENDENTE_SEGUNDOS), independente de quantas requisições
    aconteceram nesse meio-tempo -- ver iniciar_mfa_pendente() e
    _mfa_pendente_expirado() acima. Isso fecha a janela em que um
    cookie de sessão em mfa_pendente ficava efetivamente "vivo para
    sempre" contanto que algo continuasse tocando a sessão (ex:
    /auth/status sendo consultado ao reabrir a página), sem o usuário
    nunca ter provado a senha de novo.
    """
    @wraps(f)
    def wrapper(*args, **kwargs):
...
def requer_login_ou_onboarding_pendente(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        # _checagem_base_sessao, já importada/definida no módulo).
        erro = _checagem_base_sessao()
        if erro:
            return erro
        _popula_g()
