"""
Fachada de estatísticas do Bion (`EstatisticasService`).

Agrega os serviços individuais do pacote `services` (um por área do
domínio) e expõe um método por métrica do dashboard, organizados pelos
grupos do catálogo de métricas:

- A1-A5: operacional (volume, tempo médio, taxa de conclusão, efetivo, engajamento)
- B1-B4: qualidade da IA (confiança, completude, versões em uso)
- C1-C5: epidemiológico (CID por região, evolução temporal, incidência, tempo até atendimento, queixas)
- D1-D4: farmacovigilância (interações, alergias, urgência de exames, medicamentos por classe)
- E2-E3: pitch executivo (tendência de eficiência, correlação completude x confiança)
- F1-F4: histórico clínico do paciente (doenças crônicas, uso contínuo, tipo sanguíneo, gravidade de alergias)

Cada método aqui é um repasse fino para o serviço correspondente —
a lógica de cálculo e interpretação vive nos módulos individuais.
"""
