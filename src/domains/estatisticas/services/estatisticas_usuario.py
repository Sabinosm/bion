"""Estatísticas da equipe (usuários/profissionais): contagens, efetivo
ativo por papel e engajamento/atividade recente."""
...
class EstatisticasUsuario:
    ...
    def efetivo_ativo(self, id_empresa):
        """Contagem de usuários ativos por papel (médico, enfermeiro,
        admin) (A4).
    ...
    def engajamento_equipe(self, id_empresa, dias=7):
        """Quantidade de profissionais sem acesso ao sistema há mais de N
        dias (A5).

        `direcao` alto_ruim (mais inativos é pior). A comparação NÃO é
        vs. período anterior aqui (exigiria snapshot histórico de quem
        estava inativo em cada data, que não existe) — em vez disso, é
        o % de inativos sobre o total de profissionais ativos, que já é
        uma leitura relativa útil.
