"""Estatísticas de prescrição de exames: cruzamento de urgência x origem
da sugestão (IA vs. profissional)."""

from src.domains.estatisticas.interpretacao_helper import interpretacao_sem_nivel
from src.domains.prescricao.prescricao_exame.prescricao_exame_service import PrescricaoExameService
...
class EstatisticasPrescricaoExame:
    ...
    def urgencia_por_origem(self, id_empresa, dias=30):
        """Cruzamento urgência x origem_sugestao, com % de participação da
        IA em cada nível de urgência (D3).

        Sem nível ou comparação: é uma matriz cruzada (urgência x
        origem), não um valor único — comparar a matriz inteira com o
        período anterior polui mais do que ajuda. `direcao` neutra: maior
        % de sugestão pela IA não é bom nem ruim por si só.
